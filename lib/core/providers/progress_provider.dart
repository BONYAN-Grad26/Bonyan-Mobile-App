...
class ProgressProvider extends ChangeNotifier {
  ...
  bool isExerciseCompleted(int workoutId, String exerciseName) {
    return prefs.getBool('exercise_${workoutId}_$exerciseName') ?? false;
  }

  Future<void> toggleExercise(int workoutId, String exerciseName, bool value) async {
    await prefs.setBool('exercise_${workoutId}_$exerciseName', value);
    notifyListeners();
  }
  ...
  Future<void> clearProgress() async {
    ...
    for (String key in keys) {
      if (key.startsWith('meal_') || key.startsWith('workout_') || key.startsWith('exercise_')) {
